...
<div class="container mt-4">
    ...
    <form id="formCrearCancion" action="{{ route('dashboard.canciones.store') }}" method="POST" data-redirect="{{ route('dashboard.canciones.index') }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="titulo_cancion" id="titulo_cancion" class="form-control" value="{{ old('titulo_cancion') }}" required>
        </div>

        <div class="mb-3">
            <label for="duracion" class="form-label">Duración</label>
            <input type="text" name="duracion" id="duracion" class="form-control" value="{{ old('duracion') }}" required placeholder="Ej: 03:30">
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="id_album" id="id_album" class="form-select" required>
                ...
                @foreach($albumes as $album)
                    <option value="{{ $album->id_album }}" {{ old('id_album') == $album->id_album ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            ...
            <select name="artistas_colaboradores[]" id="artistas_colaboradores" class="form-select" multiple>
                @foreach($artistas as $artista)
                    <option value="{{ $artista->id_artista }}" {{ (collect(old('artistas_colaboradores'))->contains($artista->id_artista)) ? 'selected' : '' }}>
                        {{ $artista->n_artista }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="generos[]" id="generos" class="form-select" multiple>
                @foreach($generos as $genero)
                    <option value="{{ $genero->id_gen }}" {{ (collect(old('generos'))->contains($genero->id_gen)) ? 'selected' : '' }}>
                        {{ $genero->n_genero }}
                    </option>
                @endforeach
            </select>
